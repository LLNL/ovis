...
typedef int (*stat_printer_f)(nvlist_t *, const char *, zpool_handle_t *zhp);
    ...
    const char *pool_name, int descend, zpool_handle_t *zhp)
{
        ...
        if (descend && nvlist_lookup_nvlist_array(nvroot, ZPOOL_CONFIG_CHILDREN,
			...
			err = func(child[c], pool_name, zhp);
                }
        }
        return (err);
}
...
int get_stats(zpool_handle_t *zhp, void *data)
{
        ...
        err = get_recursive_stats(get_vdev_stats, nvroot, pool_name, 1, zhp);
        /*free(pool_name);*/
        zpool_close(zhp);
        return (err);
}
