...
static struct ldms_metric_template_s zfs_leafvdevs[] = {
    ...
    {"vdevpath",                 0, LDMS_V_CHAR_ARRAY, "", MAX_ZFS_NAME_LEN },
    {"vdevguid",                 0, LDMS_V_CHAR_ARRAY, "", MAX_ZFS_NAME_LEN },
...
static vdevs_count_t zpool_vdev_list_len; /* Aggregated number of vdev per zpool */
...
static int initialize_ldms_structs()
{
	...
	zpool_vdev_list_len.counttype  = LEAFVDEV_COUNT;
		...
		zpool_handle_t *zhp)
{
	...
	vdev_path = zpool_vdev_name(g_zfs, zhp, nvroot, VDEV_NAME_PATH | VDEV_NAME_FOLLOW_LINKS);
...
static int get_recursive_stats(stat_printer_f func, nvlist_t *nvroot,
    const char *pool_name, int descend, zpool_handle_t *zhp)
{
        uint_t c, children;
        nvlist_t **child;
        int err;
        ...
        if (descend && nvlist_lookup_nvlist_array(nvroot, ZPOOL_CONFIG_CHILDREN,
			...
			get_recursive_stats(func, child[c], pool_name, descend,
					zhp);
		}
        } else {
